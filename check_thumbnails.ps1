            ...
            $projectIssues.MissingEdit | ForEach-Object {
                ...
                try {
                    ...
                    for ($i = 1; $i -le 10; $i++) {
                        ...
                        $vPathBatch = $videoPath.Replace('%', '%%')
                        $tPathBatch = $thumbPath.Replace('%', '%%')
                        $fixCommands += "ffmpeg -y -noautorotate -err_detect ignore_err -fflags +genpts+igndts+discardcorrupt -color_primaries 1 -color_trc 1 -colorspace 1 -bsf:v h264_metadata=colour_primaries=1:transfer_characteristics=1:matrix_coefficients=1 -ss $timestamp -analyzeduration 2147483647 -probesize 2147483647 -i `"$vPathBatch`" -map 0:v:0 -an -update 1 -vframes 1 -vf `"setparams=color_primaries=bt709:color_trc=bt709:colorspace=bt709,scale=${thumbWidth}:${thumbHeight}:force_original_aspect_ratio=decrease:flags=lanczos+accurate_rnd,format=yuv420p`" -pix_fmt yuv420p -color_range 2 -colorspace 1 -color_trc 1 -color_primaries 1 -map_metadata -1 -strict -2 `"$tPathBatch`" 2>`"%TEMP%\fferr.log`" || (echo `"$vPathBatch`" >> `"%FAILED_LIST%`" && echo [TECHNICAL DIAGNOSTICS] >> `"%FAILED_REPORT%`" && ffprobe -v error -show_entries stream=codec_name,pix_fmt,width,height,color_range,color_space,color_transfer,color_primaries -of default=noprint_wrappers=1 `"$vPathBatch`" >> `"%FAILED_REPORT%`" && echo [FFMPEG ERROR LOG] >> `"%FAILED_REPORT%`" && type `"%TEMP%\fferr.log`" >> `"%FAILED_REPORT%`" && echo. >> `"%FAILED_REPORT%`" && echo -------------------------------------------------- >> `"%FAILED_REPORT%`")"
                    }
                } catch {
                ...
                if ($video) {
                    ...
                    if ($thumbPath.Contains('Edit Thumbnails')) {
                        try {
                            ...
                            $fixCommands += "ffmpeg -y -noautorotate -err_detect ignore_err -fflags +genpts+igndts+discardcorrupt -color_primaries 1 -color_trc 1 -colorspace 1 -bsf:v h264_metadata=colour_primaries=1:transfer_characteristics=1:matrix_coefficients=1 -ss $timestamp_recalc -analyzeduration 2147483647 -probesize 2147483647 -i `"$vPathBatch`" -map 0:v:0 -an -update 1 -vframes 1 -vf `"setparams=color_primaries=bt709:color_trc=bt709:colorspace=bt709,scale=${thumbWidth}:${thumbHeight}:force_original_aspect_ratio=decrease:flags=lanczos+accurate_rnd,format=yuv420p`" -pix_fmt yuv420p -color_range 2 -colorspace 1 -color_trc 1 -color_primaries 1 -map_metadata -1 -strict -2 `"$tPathBatch`" 2>`"%TEMP%\fferr.log`" || (echo `"$vPathBatch`" >> `"%FAILED_LIST%`" && echo [TECHNICAL DIAGNOSTICS] >> `"%FAILED_REPORT%`" && ffprobe -v error -show_entries stream=codec_name,pix_fmt,width,height,color_range,color_space,color_transfer,color_primaries -of default=noprint_wrappers=1 `"$vPathBatch`" >> `"%FAILED_REPORT%`" && echo [FFMPEG ERROR LOG] >> `"%FAILED_REPORT%`" && type `"%TEMP%\fferr.log`" >> `"%FAILED_REPORT%`" && echo. >> `"%FAILED_REPORT%`" && echo -------------------------------------------------- >> `"%FAILED_REPORT%`")"
                        } catch {
